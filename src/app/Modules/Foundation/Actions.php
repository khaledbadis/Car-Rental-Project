...
class Actions
{
    ...
    public function saveStaff(User $actor, array $input, ?int $id = null): User
    {
        ...
        return DB::transaction(function () use ($actor, $input, $id) {
            ...
            $v = Validator::make($input, ['name' => 'required|string|max:120', 'email' => ['required', 'email', 'max:190', Rule::unique('users')->ignore($user->id)], 'role' => ['required', Rule::in(array_keys(config('access.roles')))], 'active' => 'required|boolean', 'password' => [$id ? 'nullable' : 'required', 'string', 'min:12', 'max:128'], 'reason' => 'required|string|max:500'])->validate();
    ...
    {
        $v = Validator::make($input, ['locale' => ['required', Rule::in(['fr', 'ar', 'en'])], 'theme' => ['required', Rule::in(['light', 'dark', 'system'])]])->validate();
        $actor->update($v);
    ...
    {
        $v = Validator::make($input, ['current_password' => 'required|string', 'password' => 'required|string|min:12|max:128|confirmed'])->validate();
    ...
    {
        Gate::forUser($actor)->authorize('settings.manage');
        $v = Validator::make($input, ['preparation_minutes' => 'required|integer|min:0|max:1440', 'late_grace_minutes' => 'required|integer|min:0|max:1440', 'no_show_minutes' => 'required|integer|min:0|max:1440', 'upload_limit_mb' => 'required|integer|min:1|max:10', 'reason' => 'required|string|max:500'])->validate();
